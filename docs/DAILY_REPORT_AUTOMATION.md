...
기능성 평가표의 지표명은 `한국어(영문명 또는 약어)` 형식으로 표시하고, 결과는 `대조군 대비 증가·감소·개선(군간 차이)` 또는 `기준선 대비 증가·감소·개선(군내 변화)` 형식으로 정규화한다. p값은 별도 열에 원문 수치로 표시하며 근거 위치는 Page 번호 대신 Figure 또는 Table 번호만 사용한다. 연결 근거번호를 확인하지 못한 항목은 `세부 확인 필요`로 표시한다.

안전성 영역은 식물성 원재료의 식품원료목록 등재 여부와 자동 조회 가능한 DB 결과만 제공한다. 독성시험, 이상사례, 취약군, 상호작용 및 섭취량별 안전성처럼 자동 조회만으로 확정할 수 없는 항목은 `세부 확인 필요`로 구분한다. 개발·규제 실행안은 상세 보고서에 노출하지 않는다.
...
- 통계 검정법은 시험설계에 한 번만 표시한다. 기능성 결과표는 동일 영역을 세로 병합하고 결과값·p값·근거 위치만 반복 없이 표시한다.
- 유의수준은 원문 기준을 우선하며 별도 기준이 없을 때 `p < 0.05`인 p값만 강조한다.
- 주요 결과 시각자료는 `Table 2`, `Figure 3`처럼 구조화된 근거 위치를 우선 사용하고, 페이지 위치만 있으면 평가변수·결과 키워드와 원문 legend의 일치도를 사용한다.
- 원문 Figure 이미지를 변형하거나 재작성하지 않는다. Table은 PMC 원문 요소 캡처를 우선하며 접근 실패 시에만 원문 XML 구조를 SVG로 재구성한다.
